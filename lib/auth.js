...
function getOAuthClientId() {
  ...
  const manifestClientId = configuredSecret(
    manifest.oauth2?.client_id || manifest.config?.GOOGLE_OAUTH_CLIENT_ID
  );
  const stored = configuredSecret(readStoredSync('googleOAuthClientId'));
  return manifestClientId || stored || '';
}
...
async function requestAccessToken({ prompt = 'select_account consent' } = {}) {
  ...
  if (!isClientIdConfigured()) {
    throw new Error('Chưa cấu hình Google OAuth Web Client ID trong manifest.json hoặc Settings.');
  }
...
export async function loginNewUser() {
  if (!isClientIdConfigured()) {
    throw new Error('OAuth Web Client ID chưa cấu hình. Thêm client_id vào manifest.json hoặc nhập trong Settings.');
  }
